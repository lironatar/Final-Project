...
const generateUserComponentEntry = (admin, entryPath) => {
  ...
  const componentsPart = Object.keys(UserComponents).map(componentId => [`import ${componentId} from '${path.relative(absoluteEntryPath, UserComponents[componentId])}'`, `AdminBro.UserComponents.${componentId} = ${componentId}`].join('\n')).join('\n');
