...
describe('PropertyDecorator', function () {
  ...
  describe('#isSortable', function () {
    it('passes the execution to the base property', function () {
      ...
      expect(new _propertyDecorator.default(this.args).isSortable()).to.equal(false);
    });
  });
  describe('#isVisible', function () {
    it('passess execution to BaseProperty.isVisible for list when no options are specified', function () {
      expect(new _propertyDecorator.default(this.args).isVisible('list')).to.equal(this.property.isVisible());
    });
    it('passess execution to BaseProperty.isEditable for edit when no options are specified', function () {
      ...
      expect(new _propertyDecorator.default(this.args).isVisible('edit')).to.equal(this.property.isEditable());
    });
  ...
  fields.forEach(field => {
    describe(`#${field}`, function () {
      it('passess the execution to the overrideFromOptions', function () {
        ...
        new _propertyDecorator.default(this.args)[field]();
  ...
  describe('#label', function () {
    it('returns camelcased name', function () {
      ...
      expect(new _propertyDecorator.default(this.args).label()).to.equal('Normalname');
    });
  });
  describe('#availableValues', function () {
    it('map default value to { value, label } object', function () {
      ...
      expect(new _propertyDecorator.default(this.args).availableValues()).to.deep.equal([{
        value: 'val',
        label: 'val'
      }]);
    });
  ...
  describe('#position', function () {
    it('returns -1 for title field', function () {
      ...
      expect(new _propertyDecorator.default(this.args).position()).to.equal(-1);
    });
    it('returns 100 for all other fields', function () {
      this.sinon.stub(_baseProperty.default.prototype, 'isTitle').returns(false);
      expect(new _propertyDecorator.default(this.args).position()).to.equal(100);
    });
  });
  describe('#toJSON', function () {
    it('returns JSON representation of a property', function () {
      expect(new _propertyDecorator.default(this.args).toJSON()).to.have.keys('isTitle', 'isId', 'position', 'isSortable', 'availableValues', 'name', 'label', 'type', 'reference', 'components', 'subProperties', 'isArray');
    });
  });
});